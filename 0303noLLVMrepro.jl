...
function simulate(F, T, B::Matrix, para_sim, decay_constant = 0.00001)
    ...
    #println("tspan: ", tspan)
    prob = ODEProblem(evolve!, c0, tspan, p)
...
ans = simulate(F, T, B, para_sim, 0.0022)
...
function predict_neuralode(u)
    ...
    pred = simulate(F, T, p_B, para_sim, p_fcoeff[1])
    return pred
end
