...
function simulate!(sol, tunables)
  tmp1 = simulate_with_tunables(tunables)
  print(tmp1)  
  sol[1] = tmp1[1]
  return nothing
end
...
sol = simulate_with_tunables(tmp)
dsol = make_zero(sol)
tmp = 0.1ones(length(SS.canonicalize(SS.Tunable(), p)[1]))
dtmp = make_zero(tmp)
tmp3 = SS.canonicalize(SS.Tunable(), p)[1]
tmp4 = make_zero(tmp3)

Enzyme.autodiff(Reverse, simulate!, Duplicated(sol, dsol), Duplicated(tmp, dtmp))
